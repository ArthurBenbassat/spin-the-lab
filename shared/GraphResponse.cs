...
public class GraphResponse<Type>
{
    ...
    private static Type? Send(HttpRequest outboundReq, string? content = null)
    {
        if (content != null)
        {
            Handler.Logger.LogDebug($"content: {content}");
        ...
        Handler.Logger.LogDebug($"outbound req: {JsonSerializer.Serialize(outboundReq)}");
        var response = HttpOutbound.Send(outboundReq);
        Handler.Logger.LogDebug($"outbound response: {JsonSerializer.Serialize(response)}");
